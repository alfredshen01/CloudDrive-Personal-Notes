...
> 本篇分兩部分:**第一部**說明評測系統怎麼設計,**第二部**是測試結果與分析。
...
- **頂層——真實模型評測 sweep**：以真實 gemma4 實跑,量測通過率、延遲、跳針率。
...
- **跳針**：模型重複生成同一段內容且無法停止之現象（repetition loop）,以下沿用此稱。
- **thinking**：gemma 此類模型在輸出正式答案前先生成的一段內部推理。
- **grammar 約束（結構化解碼）**：將 JSON schema 編譯為文法、於取樣時遮蔽不合法之 token,
  使輸出必然符合格式。
...
**研究方法論**：量化 → 定位根因 → 修正 → 再驗證。

1. **真模型量測（temp_sweep）**：代表案例 × N 次 × 多個 temperature,量通過率／跳針率／延遲;機率性系統不看單次。
2. **對照實驗**：疑似退化時另起 baseline 版本對照,取得基準後再定位,不臆測。
3. **單元測試不足以驗 LLM**：單元測試裡 LLM 是 mock 的,全部通過只證明管線接得對,不證明真模型做得對。
4. **全量日誌落檔再過濾**：診斷先完整記錄,不在管線裡先 tail／grep。
...
| 7/07 | 防跳針循環三＋記憶起步 | codegen 保護、DEC-033（think:false 對照實驗）、記憶 v1、多輪評測起步 |
...
- **量測工具本身之缺陷**：單一 temperature 區塊被跳針樣本拖延逾 30 分鐘,導致存取權杖過期、後續
  案例全數回傳假性 401（數據污染）。
...
| 決策 | 改良 | 解決之環節 |
|---|---|---|
| DEC-031 | `num_predict` 生成上限 + 結構化請求採低而非零之 temperature | 跳針達上限即停止,以快速失敗取代 300 秒停滯（有界失敗） |
| DEC-032 | 計畫 schema 之技能名以 enum 枚舉（依 registry 動態組成） | 幻覺技能於取樣層即無法生成 |
| DEC-033 | planner 預設 `think:false` | 移除跳針之唯一棲息地（thinking）,使跳針無從發生 |

- **三層縱深邏輯**：使其不拖垮系統（有界失敗）→ 使其有界失敗 → 使其不發生。
- **跳針為何位於 thinking**：grammar 僅約束最終之結構化答案,無法約束 thinking（自由文字）;
...
**實驗一：think:false 對照實驗**（代表案例 × 5 次,僅變更思考開／關單一變數）
...
【圖表 5：think:false 對照實驗三聯圖（通過率／跳針／延遲）】
...
四檔 temperature 下 think:false 皆 100%,可見 temperature 非正確率的直接因素;thinking 開啟則落在 85–90%。thinking 開啟另有跳針 1–4/20、平均延遲 44–66 秒、最大延遲達 300 秒;think:false 則全數 0 跳針、平均約 6 秒。

- **temperature 之判讀**：修正跳針前所見之「temperature 效應」,實為跳針傾向（temperature 愈低,greedy decoding 愈易陷入迴圈）;
  `think:false` 移除 thinking 後,各 temperature 通過率一致為 100%,可知 temperature 並非正確率之直接因素。故將
  `structured_temperature` 固定於 0.2（能打破 greedy decoding repetition loop 之最小非零 temperature）,以求輸出之確定性,
  而非因 temperature 較高而失敗。
- **模型能力上限**：雜訊清除後,困難案例集 M3/M5 之通過率仍僅約 47%。（評測案例分為 M1–M5,
...
新增「對話記憶」功能後,缺乏可重現之測試,無法證明「模型確實使用了對話歷史」。需將
「模型是否使用先前對話」轉為可量測之數字,補上此驗證缺口。
...
- **記憶接線**：規劃器載入最近 N 則歷史;工具結果經真實模型對照實驗決定以 assistant 文字承載
...
**真實使用未觸及之原因**（此為「為何需要評測」之最佳實例）：
- 使用者查看檔案時直接使用檔案總管介面,不會特意要求助理「列出根目錄」;對助理之請求多為
  針對已勾選檔案之動作（使用真實 item_id,不經此路徑）。
- 「列出後再詢問名稱」此類回想操作於日常幾乎不會發生。
- 因此缺陷潛伏於「經助理列根目錄並回想」此一真實使用甚少經過之路徑,直至評測刻意執行方
  暴露。此即評測之價值：系統化地執行真實使用不會自然觸及、卻確實存在缺陷的角落。
...
- **合成案例與真實路徑覆蓋**：評測案例為刻意構造,擅長暴露真實使用甚少經過之角落缺陷
...
- **量測前沿問題**：以多輪評測為工具,迭代 planner 對寫入意圖之提示工程,嘗試推進 M3/M5
  之 47% 上限。
...
| 案例 ID | 說明 | 出現於 |
|---|---|---|
| `storage-quota-read` | 查詢容量（跳針高危案例） | §5.4 對照實驗、temperature 掃描 |
...
| 圖表 5 | think:false 對照實驗三聯 | 長條圖 | §5.4 實驗一 |
| 圖表 6 | 可靠性旅程（決策鏈 before/after） | 雙軸折線 | §5.4 |
| 圖表 7 | 多輪記憶評測三案例 | 長條圖 | 附錄 C |
