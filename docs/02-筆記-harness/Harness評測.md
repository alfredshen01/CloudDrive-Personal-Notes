...
1. **真模型量測（temp_sweep）**：代表案例 × N 次 × 多個 temperature,量通過率／重複生成迴圈率／延遲;機率性系統不看單次。
...
- **重複生成迴圈導致逾時**：模型陷入重複生成,持續產出至讀取逾時（300 秒）而回傳 503。此為「停滯」
  之機制。
...
【圖表 5：think:false 對照實驗三聯圖（通過率／重複生成迴圈／延遲）】
【圖表 6：可靠性旅程圖（沿 DEC-031→032→033 之通過率上升、重複生成迴圈率下降）】
...
- **temperature 之判讀**：修正重複生成迴圈前所見之「temperature 效應」,實為重複生成迴圈傾向（temperature 愈低,greedy decoding 愈易陷入迴圈）;
...
100%（15/15）、0 次重複生成迴圈。此結果印證「真實診斷方為可信之判準;憑閱讀程式碼向前推論、預設上游
成功,是典型之認知陷阱」。
