...
> 狀態:分析文件(2026-06-25)。對應已實作功能見 cloud_drive 的 [tasks/external-model.md](https://github.com/billwu101/CloudDrive/blob/main/doc/tasks/external-model.md)、決策 DEC-026。
...
專案 AI 助理預設使用本機 Ollama(免費、自架);使用者可為每則訊息自選模型來源——本機或自建的具名外部連線(撰文當時的設計是「本機連續失敗就自動改用外部模型」,其後由手動選擇取代,演進見 [harness筆記](harness筆記.md) 第一部 §3)。外部連線走使用者自己的 GPT-5.5,有兩條路徑:
...
**小結**:「本機收到再送雲端」方向正確,且就是現行方案;「送」這步由使用者手動貼上(現行做法),或由使用者安裝本機程式自動傳送(並不更省事)。真正做不到的只有「完全在網頁、什麼都不用裝」。
