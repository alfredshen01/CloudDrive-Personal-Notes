...
在程式碼層,這五個 runtime 元件再細分為九個實作模組。拆分不是為了湊數、也不是每個元件平均
切開,而是**沿著各元件內部真正不同質、需要各自演化與獨立測試的接縫切**：

- **E → 兩個模組（主迴圈 while loop / 子代理 sub-agent）**：主迴圈驅動一般請求的
  「送訊息→解析→執行→回填」;子代理是為了另一種生命週期（現場生成技能）才派生的有界執行
  環境,有自己的 prompt 組裝與收斂邊界。兩者控制流不同,合成一個會讓主迴圈邏輯與 codegen
  細節糾纏,故分開、也分開測試。
- **T → 兩個模組（技能登錄機制 registry / 內建技能 built-in）**：registry 是通用的
  manifest 與 dispatch 機制,把內建、使用者自建、現場生成三種技能一視同仁;內建技能只是其中
  一種來源（具體實作）。把「機制」與「內容」分離,日後新增技能不必動到 dispatch。
- **C → 兩個模組（脈絡管理 / system prompt 組裝）**：脈絡管理處理 token 預算、裁切、工具
  輸出瘦身,與內容無關、可重用;prompt 組裝則因 agent 而異（規劃器與 codegen 各一套）,在
  程式碼中本就內嵌於各 agent、無獨立檔案。
- **S → 一個模組（session 持久化）**：此元件職責單一（依 `user_id` 隔離的持久化）,內部
  沒有異質接縫,故不拆。這說明「六對九」不是每個都拆——不需要拆的就不拆。
- **L → 兩個模組（治理攔截點 hooks / 強制機制 permissions & safety）**：hooks 界定
  「在哪裡」治理（攔截點）;permissions/codeguard/sandbox 界定「治理什麼」（強制機制）。
  分開後,新增一個攔截點不必重寫權限邏輯,權限與沙箱也能各自測試。

下表彙整六元件如何對應到九個引擎模組（拆分理由見上方各條）：

| 核心元件 | 引擎模組數 |
|---|---|
| E Execution Loop | 2 |
| T Tool / Skill Registry | 2 |
| C Context Manager | 2 |
| S State Store | 1 |
| L Lifecycle Hooks | 2 |
| 合計 | 9 |

（V Evaluation Interface 是離線評測工具,不在請求路徑上,不屬這九個引擎模組。）九個模組不在
同一抽象層級——有主元件（while loop）、有子來源（built-in 是 registry 的技能來源之一）、也有
橫切治理（permissions & safety）。這就是「概念上分六層、程式碼上分九層」的實質:分層依測試與
演化邊界,而非另立一套架構。（後文 `DEC-NNN` 為本專案的設計決策編號,完整清單見 §7。）
